...
export function UiModal({
  ...
  const handleClickOutside = function (event) {
    if (event.target.closest('[data-id="modal-content"]')) {
      console.log("click inside modal content container");
    } else onClose();
  };
  const modal = (
    <div
      ...
      className={clsx(
        "fixed overflow-y-auto inset-0 bg-slate-900/60 backdrop-blur pt-10 pb-10",
        className,
      )}
    >
      <div
      ...
      >
        ...
        <button
          onClick={onClose}
          className="w-8 h-8 flex items-center justify-center text-white rounded-md bg-white/10 hover:bg-white/40 transition-colors absolute top-0 -right-10"
        >
          <CrossIconLight />
        </button>
      </div>
  ...
  return isOpen && createPortal(modal, document.querySelector("#modal"));
}

UiModal.Header = function Header({ className, children }) {
  return <div className={clsx("px-6 pt-6 pb-4", className)}>{children}</div>;
};

UiModal.Body = function Body({ className, children }) {
  return <div className={clsx("px-6", className)}>{children}</div>;
};

UiModal.Footer = function Footer({ className = "", children }) {
  return (
    <div className={clsx("p-6 gap-4 flex justify-end mt-auto", className)}>
      {children}
    </div>
  );
};
